...
export default async function handler(req:any,res:any){
  ...
  try{
    ...
    const sql=neon(url)
    const link=await sql`select token,form_code,recipient_name,recipient_email,function_name,recipient_role,active,expires_at from shared_surveys where token=${token} limit 1`
    ...
    await sql`insert into survey_responses (form_code, respondent_token, respondent_name, respondent_email, payload, submitted_at, source)
      values (${formCode}, ${token}, ${name}, ${email}, ${JSON.stringify(cleanPayload)}, now(), 'PUBLIC_LINK')`
